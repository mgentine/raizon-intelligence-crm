...
import { bulkUpsertCompanies, createActivity, createCompany, createContact, createImportRun, completeRecurringItem, createOpportunity, createRecurringItem, createRegulatoryAct, createUnit, finishImportRun, getDashboardStats, getCetesbLeadStatus, listCompanies, listContacts, listImportRuns, listNotifications, listOpportunities, listRecentActivities, listRegulatoryActs, listUpcomingRecurring, listUnits, markNotificationRead, updateOpportunityStage } from "./db";
...
export const appRouter = router({
  ...
  recurring: router({
    ...
    complete: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(({ ctx, input }) => completeRecurringItem(input.id, ctx.user.id)),
  }),
});
