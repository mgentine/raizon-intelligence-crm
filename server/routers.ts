...
export const appRouter = router({
  ...
  opportunities: router({
    ...
    updateStage: protectedProcedure.input(z.object({ id: z.number().int().positive(), stage: z.enum(["new", "enrichment", "actionable", "contacted", "qualified", "diagnosis", "scoping", "proposal", "negotiation", "approved", "won", "lost", "nurture", "discarded"]), lossReason: z.string().trim().min(2).optional() }).refine((input) => input.stage !== "lost" || Boolean(input.lossReason), { message: "Informe o motivo da perda." })).mutation(({ ctx, input }) => { requireProfile(ctx, ["admin", "commercial"]); return updateOpportunityStageWithLossReason(input.id, input.stage, input.lossReason); }),
