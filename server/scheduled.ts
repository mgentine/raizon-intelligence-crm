...
export async function refreshRegulatoryPriorities(req: Request, res: Response) {
  ...
  try {
    ...
    const followUps = await runProposalFollowUps();
    ...
    return res.json({ ok: true, taskUid: user.taskUid, refreshed: result[0].affectedRows ?? 0, notificationsCreated: notifications.created, followUps, email, sourceUpdates, blockedAttemptIds, startedAt, finishedAt: new Date().toISOString() });
  } catch (error) {
...
export async function refreshCommercialFollowUps(req: Request, res: Response) {
  try {
    const user = await sdk.authenticateRequest(req);
    if (!user.isCron || !user.taskUid) return res.status(403).json({ error: "cron-only" });
    const automationState = getOperationalAutomationState();
