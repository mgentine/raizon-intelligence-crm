...
export function ProposalCenterView() {
  ...
  const proposalSummary = useMemo(() => {
    const items = proposals.data?.map(({ proposal }) => proposal) ?? [];
    return {
      total: items.length,
      inProgress: items.filter((proposal) => ["draft", "technical_review", "commercial_review", "approved_internal", "issued", "sent", "negotiating"].includes(proposal.status)).length,
      accepted: items.filter((proposal) => proposal.status === "accepted").length,
      value: items.filter((proposal) => !["rejected", "cancelled"].includes(proposal.status)).reduce((sum, proposal) => sum + Number(proposal.investment || 0), 0),
    };
  }, [proposals.data]);
  ...
  return <div>
    ...
    {showForm && <section className="bg-white border border-[#dfe5e0] p-6 mb-6" aria-labelledby="new-proposal-title">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div><label className="text-xs font-bold text-[#65736c] block mb-1">CNPJ do cliente cadastrado *</label><Input placeholder="Digite o CNPJ ou nome para localizar" value={companySearch} onChange={(event) => setCompanySearch(event.target.value)} /><select aria-label="Cliente cadastrado por CNPJ" className="h-10 w-full border border-[#dfe5e0] bg-white px-3 text-sm mt-2" value={form.companyId} onChange={(event) => { set("companyId")(event); resetCompanyDependentFields(); }}><option value="">Selecione a empresa encontrada</option>{filteredCompanies.map((company) => <option key={company.id} value={company.id}>{formatCnpj(company.cnpj)} — {company.tradeName || company.legalName}</option>)}</select>{companySearch && !filteredCompanies.length && <p className="text-xs text-[#e13b32] mt-1">Nenhum cliente cadastrado corresponde ao CNPJ informado.</p>}</div>
    ...
    <section className="bg-white border border-[#dfe5e0] p-6"><div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5 mb-6"><div className="flex items-start gap-3"><FileText className="h-5 w-5 text-[#e13b32] mt-1" /><div><h2 className="text-xl font-black">Painel de propostas</h2><p className="text-sm text-[#65736c] mt-1">Status atual, condições comerciais e duplicação rastreável em um único lugar.</p></div></div><div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center"><div className="bg-[#f5f7f5] px-3 py-2"><strong className="block text-lg">{proposalSummary.total}</strong><span className="text-[10px] uppercase tracking-[.12em] text-[#65736c]">Total</span></div><div className="bg-[#fff1df] px-3 py-2"><strong className="block text-lg">{proposalSummary.inProgress}</strong><span className="text-[10px] uppercase tracking-[.12em] text-[#9a4d00]">Em curso</span></div><div className="bg-[#e3f6eb] px-3 py-2"><strong className="block text-lg">{proposalSummary.accepted}</strong><span className="text-[10px] uppercase tracking-[.12em] text-[#17683b]">Aceitas</span></div><div className="bg-[#f7f8f6] px-3 py-2"><strong className="block text-lg">{money(proposalSummary.value)}</strong><span className="text-[10px] uppercase tracking-[.12em] text-[#65736c]">Carteira</span></div></div></div>{proposals.isLoading && <p className="py-8 text-sm text-[#65736c]">Carregando propostas…</p>}{proposals.isError && <p className="py-8 text-sm text-[#e13b32]">Não foi possível carregar as propostas.</p>}{!proposals.isLoading && !proposals.isError && !proposals.data?.length && <div className="py-12 text-center"><ShieldCheck className="h-8 w-8 text-[#d6ded8] mx-auto mb-3" /><p className="font-bold">Nenhuma proposta cadastrada</p><p className="text-sm text-[#65736c] mt-1">Crie um rascunho a partir de uma oportunidade qualificada.</p></div>}<div className="grid grid-cols-1 xl:grid-cols-2 gap-3">{proposals.data?.map(({ proposal, company, service }) => <article key={proposal.id} className="border border-[#e2e8e4] bg-[#fbfcfb] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-black">{proposal.proposalNumber || "Rascunho"} · V{String(proposal.version).padStart(2, "0")}</h3><span className={`px-2 py-1 text-[10px] font-bold uppercase tracking-[.1em] ${statusTone[proposal.status] || "bg-[#f2f4f3] text-[#526059]"}`}>{statusLabels[proposal.status] || proposal.status}</span></div><p className="text-sm text-[#4d5c54] mt-3">{company?.tradeName || company?.legalName || `Empresa #${proposal.companyId}`}</p><p className="text-sm text-[#65736c] mt-1">{service?.name || "Serviço arquivado"} · <strong className="text-[#16221f]">{money(proposal.investment)}</strong></p></div></div><div className="mt-4 border-t border-[#e8ece9] pt-3"><p className="text-xs text-[#65736c]">{proposal.professional || "Profissional não informado"} · {proposal.paymentTerms || "Pagamento não informado"}</p><p className="text-xs text-[#87918c] mt-1">Validade: {proposal.validityDays} dias · Atualizada em {new Date(proposal.updatedAt).toLocaleDateString("pt-BR")}</p></div><div className="mt-4 flex flex-wrap items-center gap-2"><select aria-label="Status da proposta" className="h-9 min-w-0 flex-1 border border-[#dfe5e0] bg-white px-2 text-xs" value={proposal.status} onChange={(event) => updateStatus.mutate({ id: proposal.id, status: event.target.value as "draft" })}>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{!proposal.proposalNumber && (user?.role === "admin" || user?.profile === "commercial") && <Button size="sm" className="bg-[#e13b32] text-white" disabled={issue.isPending} onClick={() => issue.mutate({ id: proposal.id })}><Send className="h-4 w-4 mr-1" />Emitir</Button>}{(user?.role === "admin" || user?.profile === "commercial") && <Button size="sm" variant="outline" disabled={createVersion.isPending} onClick={() => createVersion.mutate({ id: proposal.id })}><Copy className="h-4 w-4 mr-1" />Duplicar</Button>}</div></article>)}</div></section><p className="text-sm text-[#65736c] mt-4" role="status">{feedback}</p>
  </div>;
}
